...
import 'package:nolive_app/src/app/runtime_bridges/stripchat/stripchat_mouflon_runtime_support.dart'
    as stripchat_runtime;
...
bool stripchatMp4BytesContainInitialization(Uint8List bytes) {
  return stripchat_runtime.stripchatMp4BytesContainInitialization(bytes);
}
...
class StripchatLlHlsProxy {
  ...
  Future<List<LivePlayUrl>> wrapPlayUrls({
    ...
    for (final playUrl in playUrls) {
      ...
      final session = _createSession(playUrl, keyCache: keyCache);
  ...
  _StripchatLlHlsSession _createSession(
    ...
    return _StripchatLlHlsSession(
      ...
      playlistCdnDomains: (List<String>.from(
        playUrl.metadata?['stripchatCdnDomains'] as List? ?? const <String>[],
      )).where((d) => !_stripchatTlsDeadCdnDomains.contains(
        d.trim().toLowerCase(),
  ...
  Future<void> _primeSession(_StripchatLlHlsSession session) async {
    try {
      ...
      final childUri = childUrls.isNotEmpty ? childUrls.first : masterFetched.finalUri;
  ...
  Future<_FetchedPlaylist> _fetchPlaylistWithFallbacks({
    ...
    if (primaryFetched != null && primaryFetched.statusCode != HttpStatus.forbidden) {
    ...
    for (final fallbackUri in fallbackUris) {
      ...
      if (fallback.statusCode < HttpStatus.badRequest) {
        _trace(
          ...
          'from=${
              (primaryFetched?.finalUri ?? uri).host
            } to=${fallback.finalUri.host}',
        );
        return fallback;
      }
    }
    ...
    for (final rawLine in const LineSplitter().convert(manifest)) {
      ...
      if (line.startsWith('#EXT-X-MOUFLON:PSCH:')) {
        ...
        if (auth == null) {
          auth = parsed;
          if (kDebugMode) {
            debugPrint('[psch] auth init pkey=${redact(parsed?.pkey)}');
          }
        } else if (parsed != null) {
          ...
          if (!currentKnown) {
            ...
            debugPrint('[psch] auth kept known=${redact(auth.pkey)}, skip ${redact(pkey)}');
          }
        }
        continue;
      }
      if (line.startsWith('#EXT-X-MOUFLON:URI:')) {
        final resolvedTargets = _resolveMouflonAssetTargets(
      ...
      if (target.path.toLowerCase().endsWith('.m3u8')) {
        final rawVariantUri =
            Uri.tryParse(playlistUri.resolve(line.trim()).toString());
        final variantPkey =
  ...
  Future<bool> _shouldDropMapForPlaylist({
    ...
    final rawCandidates = (firstSegmentFallbacks != null && firstSegmentFallbacks.isNotEmpty)
        ? firstSegmentFallbacks
        : <Uri>[firstSegmentTarget];

    final candidates = rawCandidates.fold<List<Uri>>(<Uri>[], (list, candidate) {
    ...
    Future<bool> checkCandidate(Uri candidate) async {
      try {
        ...
        try {
          ...
          return stripchatMp4BytesContainInitialization(bytes);
        } finally {
          if (response.statusCode != HttpStatus.ok &&
              response.statusCode != HttpStatus.partialContent) {
            await response.drain<void>().timeout(const Duration(seconds: 1)).catchError((_) {});
          }
        }
      } catch (_) {
        return false;
  ...
  Uri _localProxyUri(
    ...
    final assetUris =
        fallbackTargets == null || fallbackTargets.isEmpty
        ? <Uri>[target]
        : <Uri>[...fallbackTargets];
  ...
  void _trace(String message) {
    ...
    AppLog.instance.info('stripchat/proxy', redacted);
  }
  ...
  StripchatLlHlsProxyTestSession createSessionForTest(
    ...
    return StripchatLlHlsProxyTestSession(
      _createSession(playUrl, keyCache: keyCache),
    );
  }
...
_StripchatResolvedAssetTargets _resolveMouflonAssetTargets({
  ...
  final decoded = _decodeMouflonUri(
    rawUri,
...
_DecodedMouflonUri _decodeMouflonUri(
  String rawUri, {
  required List<String> candidatePkeys,
  required StripchatMouflonKeyCache keyCache,
  void Function(String phase, {required String pkey, required String source})?
  ...
  if (kDebugMode) {
    ...
    debugPrint('[pdkey-seg] encryptedSegment=${encryptedMatch.encryptedSegment}');
  }
  ...
  String? tryDecrypt(String pkey, String pdkey) {
    final result = _decryptMouflonSegment(encryptedSegment, pdkey);
  ...
  for (final normalizedPkey in normalizedPkeys) {
    ...
    if (record != null && !_shouldIgnorePdkeyRecord(record)) {
      ...
      final decryptedSegment = tryDecrypt(normalizedPkey, record.pdkey);
    ...
    if (hardcodedPdkey != null &&
      ...
      final decryptedSegment = tryDecrypt(normalizedPkey, hardcodedPdkey);
      if (decryptedSegment != null) {
        tracePdkey?.call(
          'resolved',
          pkey: normalizedPkey,
...
String? _decryptMouflonSegment(String encryptedSegment, String pdkey) {
  try {
