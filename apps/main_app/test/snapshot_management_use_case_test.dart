...
void main() {
  ...
  test('snapshot management imports legacy-compatible config export', () async {
    ...
    expect(importedSnapshot.blockedKeywords, unorderedEquals(['剧透', '广告']));
    expect(bootstrap.themeMode.value, ThemeMode.dark);
    expect(bootstrap.followWatchlistSnapshot.value, isNull);
  });

  test(
    'snapshot management rejects malformed import json with format error',
    () async {
      final bootstrap = createAppBootstrap(mode: AppRuntimeMode.preview);

      await expectLater(
        () => bootstrap.importSyncSnapshotJson('{"settings":'),
  ...
  test(
    'snapshot management maps legacy config keys to current settings',
    () async {
      final bootstrap = createAppBootstrap(mode: AppRuntimeMode.preview);
      ...
      expect(
        snapshot.settings.containsKey('account_chaturbate_cookie'),
        isFalse,
      );
      ...
      expect(
        accountSettings.chaturbateCookie,
      ...
      expect(layoutPreferences.providerOrder.take(3).toList(), [
        'douyin',
        'chaturbate',
        'douyu',
      ]);
      ...
      expect(followPreferences.displayMode, FollowDisplayModePreference.list);
    ...
    () async {
      ...
      await bootstrap.updateSyncPreferences(
        const SyncPreferences(
          ...
          localPeerAddress: '',
          localPeerPort: 23234,
          localPeerAccessToken: 'peer-sync-token',
        ),
      );
      ...
      expect(snapshot.settings.containsKey('account_youtube_cookie'), isFalse);
      expect(snapshot.settings.containsKey('sync_webdav_password'), isFalse);
      expect(snapshot.settings.containsKey('sync_local_access_token'), isFalse);
      expect(
        snapshot.settings.containsKey('sync_local_peer_access_token'),
        isFalse,
      );
      expect(snapshot.settings['account_bilibili_user_id'], 10086);
      expect(
        snapshot.settings['sync_webdav_base_url'],
      ...
      expect(settings.containsKey('sync_local_access_token'), isFalse);
      expect(settings.containsKey('sync_local_peer_access_token'), isFalse);

      final accounts = await bootstrap.loadProviderAccountSettings();
      expect(accounts.bilibiliCookie, 'SESSDATA=demo;bili_jct=test;');
      expect(
        accounts.chaturbateCookie,
        'cf_clearance=demo-clearance; __cf_bm=demo-bm',
      );
  ...
  test(
    'reset app data preserves secure credentials until explicitly cleared',
    () async {
      final bootstrap = createAppBootstrap(mode: AppRuntimeMode.preview);
  ...
  test(
    'controlled migration bundle round-trips sensitive credentials',
    () async {
      final bootstrap = createAppBootstrap(mode: AppRuntimeMode.preview);
  ...
  test(
    ...
    () async {
      ...
      expect(
        (await bootstrap.loadSyncSnapshot()).settings['theme_mode'],
        'dark',
      );
    },
  );
  ...
  test(
    'clear follows resets follow snapshot to empty and bumps revision',
    () async {
      final bootstrap = createAppBootstrap(mode: AppRuntimeMode.preview);
      await bootstrap.toggleFollowRoom(
        providerId: ProviderId.bilibili.value,
  ...
  test(
    ...
    () async {
      ...
      expect(
        (decoded['settings'] as Map<String, dynamic>)['theme_mode'],
        'dark',
      );
      expect(decoded['blocked_keywords'], contains('广告'));
      expect(decoded['tags'], contains('夜班'));
      expect(
        (decoded['history'] as List<dynamic>).single,
      ...
      final importedBootstrap = createAppBootstrap(
      ...
      final importedLayoutPreferences = await importedBootstrap
  ...
  test('webdav use cases close services after success and failure', () async {
    final preferences = _testSyncPreferences();

    final verifyService = _RecordingWebDavBackupService();
    await VerifyWebDavConnectionUseCase(createService: (_) => verifyService)(
      preferences,
    );
    expect(verifyService.testConnectionCount, 1);
    expect(verifyService.closeCount, 1);
    expect(verifyService.closeForceValues, [isTrue]);

    final uploadService = _RecordingWebDavBackupService();
    ...
    await UploadWebDavSnapshotUseCase(
      snapshotService,
      createService: (_) => uploadService,
    )(preferences);
    expect(uploadService.uploadedSnapshots, hasLength(1));
    expect(uploadService.closeCount, 1);

    final failingRestoreService = _RecordingWebDavBackupService(
      restoreError: StateError('restore failed'),
    );
    final restoreUseCase = RestoreWebDavSnapshotUseCase(
      _createSnapshotImportCoordinator(),
      createService: (_) => failingRestoreService,
    );
    await expectLater(
      () => restoreUseCase(preferences),
      throwsA(isA<StateError>()),
    );
    expect(failingRestoreService.closeCount, 1);
    expect(failingRestoreService.closeForceValues, [isTrue]);
  });
}

SyncPreferences _testSyncPreferences() {
...
SecureSnapshotImportCoordinator _createSnapshotImportCoordinator() {
  return SecureSnapshotImportCoordinator(
    snapshotService: RepositorySyncSnapshotService(
      settingsRepository: InMemorySettingsRepository(),
      historyRepository: InMemoryHistoryRepository(),
      followRepository: InMemoryFollowRepository(),
      tagRepository: InMemoryTagRepository(),
...
class _RecordingWebDavBackupService implements WebDavBackupService {
  _RecordingWebDavBackupService({this.restoreError});

  final Object? restoreError;

  int testConnectionCount = 0;
  int closeCount = 0;
  final List<bool> closeForceValues = <bool>[];
  final List<SyncSnapshot> uploadedSnapshots = <SyncSnapshot>[];
