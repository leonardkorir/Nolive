...
void main() {
  test(
    'formatUserFacingError maps socket-like errors to Chinese network copy',
    () {
  ...
  test(
    'formatUserFacingError maps timeout and keeps short Chinese passthrough',
    () {
      expect(
        formatUserFacingError(Exception('TimeoutException after 0:00:10')),
        contains('超时'),
      );
      expect(formatUserFacingError('暂时没有内容'), '暂时没有内容');
    },
  );
