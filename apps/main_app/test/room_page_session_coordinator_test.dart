...
void main() {
  ...
  test(
    ...
    () async {
      ...
      harness.ancillaryController.onLoad =
          ({required snapshot, required fallbackIsFollowed}) {
      ...
      await expectLater(coordinator.refreshRoom(), throwsA(isA<StateError>()));
      await _flushAsyncWork();
  ...
  test(
    ...
    () async {
      ...
      harness.danmakuController.onBind =
          ({required activeRoomDetail, required session}) {
  ...
  test(
    ...
    () async {
      ...
      await _flushAsyncWork();
      expect(harness.playbackController.waitReasons, <String>[
        'cleanup playback',
      ]);
  ...
  test(
    'page session coordinator initial load waits for queued runtime cleanup',
    () async {
      final harness = _RoomPageSessionHarness();
      addTearDown(harness.dispose);
      final coordinator = harness.createCoordinator();
      ...
      expect(
        harness.traces,
        containsAllInOrder(<String>['load waiting for pending cleanup']),
      );
  ...
  test(
    'page session coordinator defers initial load until lifecycle resumes',
    () async {
      final harness = _RoomPageSessionHarness();
      addTearDown(harness.dispose);
      final coordinator = harness.createCoordinator(
      ...
      expect(harness.sessionController.loadCalls, 0);
      expect(harness.playbackController.scheduledBootstraps, isEmpty);
      expect(
        harness.traces,
        contains('load waiting for resumed lifecycle state=inactive'),
      );
      ...
      expect(harness.sessionController.loadCalls, 1);
      expect(harness.playbackController.scheduledBootstraps, hasLength(1));
      expect(harness.traces, contains('load resumed lifecycle released'));
    },
  );

  test(
    'page session coordinator aborts deferred initial load after dispose',
    () async {
      final harness = _RoomPageSessionHarness();
      addTearDown(harness.dispose);
      final coordinator = harness.createCoordinator(
      ...
      final loadExpectation = expectLater(
        coordinator.startInitialLoad(),
        throwsA(isA<RoomSessionCancelledException>()),
      );
      await _flushAsyncWork();
  ...
  test(
    'page session coordinator resets leaving state when cleanup fails',
    () async {
      final harness = _RoomPageSessionHarness();
      addTearDown(harness.dispose);
      final coordinator = harness.createCoordinator();
      ...
      await expectLater(coordinator.leaveRoom(), throwsA(isA<StateError>()));
  ...
  test(
    'page session coordinator suppresses leave cleanup failure after dispose',
    () async {
      final harness = _RoomPageSessionHarness();
      addTearDown(harness.dispose);
      final coordinator = harness.createCoordinator();
      ...
      await expectLater(leavingFuture, completes);
      expect(harness.fullscreenController.cleanupCalls, 1);
    },
  );

  test(
    'page session coordinator refreshes lifecycle restore playback source',
    () async {
      final harness = _RoomPageSessionHarness();
      addTearDown(harness.dispose);
      final coordinator = harness.createCoordinator();
      addTearDown(coordinator.dispose);
      harness.sessionController.nextLoadResult = harness.primaryResult;
      harness.sessionController.nextResolvedPlaybackRefresh =
          ResolvedPlaySource(
      ...
      final playbackSource = await coordinator
  ...
  test(
    ...
    () async {
      ...
      final nextPlayerPreferences = harness.primaryResult.playerPreferences
          .copyWith(
...
class _RoomPageSessionHarness {
  ...
  static final primaryQuality = LivePlayQuality(
    id: '720',
    label: '高清',
    sortOrder: 720,
  );
  static final secondaryQuality = LivePlayQuality(
    id: '1080',
    label: '原画',
    sortOrder: 1080,
  );
  ...
  RoomSessionLoadResult _buildResult({
    ...
    final snapshot = LoadedRoomSnapshot(
      ...
      detail: room,
      qualities: <LivePlayQuality>[primaryQuality, secondaryQuality],
...
class _TestRoomAncillaryController extends RoomAncillaryController {
  _TestRoomAncillaryController(RoomAncillaryDependencies dependencies)
    : super(dependencies: dependencies, providerId: ProviderId.bilibili);
...
class _TestRoomDanmakuController extends RoomDanmakuController {
  ...
  Future<void> bindSession({
    ...
    await onBind?.call(activeRoomDetail: activeRoomDetail, session: session);
  }
