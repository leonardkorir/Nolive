...
class DouyuQuickJsSigner {
  static const String kCryptoJs = r'''
...
''';
  ...
  Future<String> sign(
    ...
    if (_disposed) {
      throw StateError('DouyuQuickJsSigner has been disposed.');
    }
    ...
    workerPort.send(<Object?>[
      _DouyuQuickJsWorkerProtocol.sign,
      requestId,
      script,
      roomId,
      deviceId,
  ...
  Future<SendPort> _ensureWorker() {
    if (_disposed) {
      return Future<SendPort>.error(
        StateError('DouyuQuickJsSigner has been disposed.'),
      );
    }
    ...
    _workerCounted = true;
    NfrIsolateTelemetry.markStarted('douyu-quickjs-signer');
    Isolate.spawn(
      _runWorker,
      receivePort.sendPort,
      debugName: 'douyu-quickjs-signer',
    ).then(
      (isolate) {
        _worker = isolate;
      },
      onError: (Object error, StackTrace stackTrace) {
        ...
        NfrIsolateTelemetry.markFailed('douyu-quickjs-signer', error);
        receivePort.close();
  ...
  void _handleWorkerMessage(Object? message) {
    ...
    if (type != _DouyuQuickJsWorkerProtocol.result ||
        message.length < 4 ||
        message[1] is! int ||
        message[2] is! bool) {
      return;
    ...
    final succeeded = message[2] as bool;
    final payload = message[3];
    if (succeeded) {
      completer.complete(payload?.toString() ?? '');
    } else {
      completer.completeError(StateError(payload?.toString() ?? 'sign failed'));
    }
  }

  void dispose() {
    _disposed = true;
    _workerPort?.send(<Object?>[_DouyuQuickJsWorkerProtocol.dispose]);
    _receivePort?.close();
  ...
  static void _runWorker(SendPort hostPort) {
    final commandPort = ReceivePort();
    final worker = _DouyuQuickJsWorker(hostPort);
    hostPort.send(<Object?>[
      _DouyuQuickJsWorkerProtocol.ready,
      commandPort.sendPort,
    ]);
    commandPort.listen((message) {
      if (message is List<Object?> &&
          message.isNotEmpty &&
          message[0] == _DouyuQuickJsWorkerProtocol.dispose) {
        worker.dispose();
        commandPort.close();
        return;
      }
...
class _DouyuQuickJsWorkerProtocol {
...
class _DouyuQuickJsWorker {
  _DouyuQuickJsWorker(this._hostPort);
  ...
  void handle(Object? message) {
    if (message is! List<Object?> ||
        message.length < 6 ||
        message[0] != _DouyuQuickJsWorkerProtocol.sign ||
        message[1] is! int) {
      return;
    }
    final requestId = message[1] as int;
    try {
      final script = message[2]?.toString() ?? '';
      final roomId = message[3]?.toString() ?? '';
      final deviceId = message[4]?.toString() ?? '';
      final timestamp = message[5]?.toString() ?? '';
      final runtime = _ensureRuntime();
      runtime.eval(script);
      final result = runtime.eval(
        "ub98484234('$roomId','$deviceId','$timestamp')",
      );
      _hostPort.send(<Object?>[
        _DouyuQuickJsWorkerProtocol.result,
        requestId,
        true,
        result,
      ]);
  ...
  JsRuntime _ensureRuntime() {
    ...
    )..eval(DouyuQuickJsSigner.kCryptoJs);
    _runtime = runtime;
    return runtime;
  }
