...
Future<void> main() async {
  ...
  try {
    final rooms = await (provider as SupportsRecommendRooms)
        .fetchRecommendRooms(page: 1);
    print('rooms=${rooms.items.length}');
    if (rooms.items.isEmpty) return;
    final room = rooms.items.first;
    print('room=${room.roomId}');
    final detail = await (provider as SupportsRoomDetail).fetchRoomDetail(
      room.roomId,
    );
    print('live=${detail.isLive} source=${detail.sourceUrl}');
    final qualities = await (provider as SupportsPlayQualities)
    ...
    final urls = await (provider as SupportsPlayUrls).fetchPlayUrls(
      detail: detail,
      quality: selected,
    );
    ...
    if (asset.isNotEmpty) {
      final assetUrl = asset.startsWith('http')
          ? asset
          : Uri.parse(play.url).resolve(asset).toString();
      ...
      print(
        'assetStatus=${resp.statusCode} bytes=${bytes.length} ct=${resp.headers.contentType}',
      );
      if (bytes.length > 16) {
        print(
          'assetMagic=${bytes.take(16).map((b) => b.toRadixString(16).padLeft(2, "0")).join(" ")}',
        );
      }
    }
    print('--- mpv ---');
