...
void main() {
  test(
    'youtube provider returns deterministic preview migration slice',
    () async {
      ...
      expect(recommend.items.first.providerId.value, 'youtube');

      final search = await provider.searchRooms('china');
      expect(search.items, isNotEmpty);
      expect(search.items.first.providerId.value, 'youtube');

      final detail = await provider.fetchRoomDetail(
        '@ChinaStreetObserver/live',
      );
      expect(detail.providerId.value, 'youtube');
      ...
      final defaultQualities = qualities.where((item) => item.isDefault);
      expect(defaultQualities, hasLength(1));
      expect(defaultQualities.single.id, '1080');
