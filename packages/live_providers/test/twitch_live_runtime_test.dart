...
void main() {
  group(
    ...
    () {
      test(
        'live twitch runtime maps recommend/search/detail/play flow',
        () async {
          ...
          expect(
            recommend.items.any((item) => item.providerId.value == 'twitch'),
            isTrue,
          );
      ...
      test(
        'live twitch runtime prefers injected playback bootstrap resolver',
        () async {
          final provider = TwitchProvider(
            dataSource: TwitchLiveDataSource(
              apiClient: _FixtureTwitchApiClient(
      ...
      test(
        'live twitch runtime does not hang on stalled alternate surfaces',
        () async {
          final provider = TwitchProvider(
            dataSource: TwitchLiveDataSource(
              apiClient: _FixtureTwitchApiClient(
...
class _FixtureTwitchApiClient implements TwitchApiClient {
  ...
  Future<Object?> postGraphQl(
    ...
    if (payload is List) {
      return [_channelShell, _streamMetadata, _viewCount, _liveBroadcast];
    }
