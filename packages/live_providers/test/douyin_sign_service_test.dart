...
void main() {
  test('douyin sign service preserves expires cookies when parsing set-cookie',
      () async {
    ...
    expect(
      headers['cookie'],
      'ttwid=from-head;msToken=token-value',
    );
  });

  test('forceRefreshCookie bypasses injected cookie and fetches a fresh value',
      () async {
    var headRequests = 0;
    final client = _FakeHttpClient((request) async {
      headRequests += 1;
      return http.Response(
        ...
        200,
        headers: const {
          'set-cookie': 'ttwid=refreshed-cookie; Path=/',
        },
      );
    });
  ...
  test('douyin sign service falls back to default cookie on refresh failure',
      () async {
    final service = HttpDouyinSignService(
