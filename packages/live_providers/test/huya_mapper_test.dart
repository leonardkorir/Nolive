...
void main() {
  group('HuyaMapper exception and edge case tests', () {
    ...
    test('offline room (state OFF) maps isLive=false without stream lines', () {
      ...
      final detail = HuyaMapper.mapRoomDetail(html, requestedRoomId: '998');
    ...
    test('offline room with empty stream blob still maps offline', () {
      final html = '''<html>
        <script>var TT_ROOM_DATA = {"state":"OFF","isReplay":false};</script>
        <script>stream: {"data":[]}</script>
      </html>''';
      final detail = HuyaMapper.mapRoomDetail(html, requestedRoomId: '660002');
      expect(detail.isLive, isFalse);
    });
