...
void main() {
  test(
    'stripchat registration stays aligned with declared capabilities',
    () async {
      ...
      final categoryRoomsContract = provider
          .requireContract<SupportsCategoryRooms>(
            ProviderCapability.categories,
          );
      expect(categoryRoomsContract, isA<SupportsCategoryRooms>());
      final subCategory = category.children.first;
      final categoryRooms = await categoryRoomsContract.fetchCategoryRooms(
        subCategory,
      );
      expect(categoryRooms.items, isNotEmpty);

      // SupportsRecommendRooms
      final recommendRoomsContract = provider
      ...
      final playQualitiesContract = provider
          .requireContract<SupportsPlayQualities>(
      ...
      final playUrls = await playUrlsContract.fetchPlayUrls(
        detail: detail,
        quality: quality,
      );
  ...
  test('ProviderId.knownValues contains stripchat', () {
    expect(ProviderId.knownValues, contains(ProviderId.stripchat));
  });
}
