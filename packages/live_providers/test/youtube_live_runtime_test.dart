...
void main() {
  group(
    ...
    () {
      test(
        'live youtube runtime maps detail and play flow from fixtures',
        () async {
          ...
          expect(recommend.items.first.providerId, ProviderId.youtube);

          final detail = await provider.fetchRoomDetail('Z3eFGbFcaXs');
          expect(detail.providerId, ProviderId.youtube);
          ...
          expect(detail.metadata?['playerClientProfile'], 'streamlink_android');
          ...
          expect(qualities.any((item) => item.label.contains('1080')), isTrue);
          ...
          expect(urls.first.lineLabel, 'Streamlink Android HLS');
          expect(urls[1].lineLabel, 'Safari HLS');
          expect(urls[2].lineLabel, 'MWEB HLS');
          expect(urls[3].lineLabel, 'iOS HLS');
          expect(urls[3].metadata?['audioUrl'], isNotNull);
          expect(urls.any((item) => item.lineLabel == 'WEB Direct'), isFalse);
          expect(urls.first.url, contains('/streamlink/1080p60.m3u8'));
          expect(urls.first.metadata?['hlsBitrate'], '6200000');
          expect(urls.first.metadata?['masterPlaylistContent'], isNull);
          expect(
            urls.first.metadata?['resolvedVariantUrl'],
            contains('streamlink/1080p60.m3u8'),
          );
          ...
          expect(autoUrls.first.lineLabel, 'Streamlink Android HLS');
          expect(autoUrls.first.url, contains('/streamlink/master.m3u8'));
        },
      );

      test(
        'keeps hls playback ahead of direct when media probe is forbidden',
        () async {
          final provider = YouTubeProvider.live(
            apiClient: _FixtureYouTubeApiClient(forbidHlsSegments: true),
          );
          ...
          expect(urls.first.lineLabel, 'Streamlink Android HLS');
          expect(urls.first.url, contains('/streamlink/master.m3u8'));
        },
      );
    },
  );
...
class _FixtureYouTubeApiClient implements YouTubeApiClient {
  _FixtureYouTubeApiClient({this.forbidHlsSegments = false})
    : _pageHtml = YouTubeFixtureLoader.loadChannelPageHtml(),
          ...
          'formats': [
            {
              ...
              'url': 'https://rr.googlevideo.com/direct/Z3eFGbFcaXs/1080p.mp4',
            },
          ],
        },
      },
      _streamlinkAndroidPlayerResponse = {
        ...YouTubeFixtureLoader.loadPlayerResponse('Z3eFGbFcaXs'),
        'playabilityStatus': const {'status': 'OK'},
        'streamingData': {
          'hlsManifestUrl':
              'https://manifest.googlevideo.com/api/manifest/hls_playlist/fixture/Z3eFGbFcaXs/streamlink/master.m3u8',
        },
      },
      _webSafariPlayerResponse = {
        ...
        'streamingData': {
          'hlsManifestUrl':
              'https://manifest.googlevideo.com/api/manifest/hls_playlist/fixture/Z3eFGbFcaXs/safari/master.m3u8',
        },
      },
      _mwebPlayerResponse = {
        ...YouTubeFixtureLoader.loadPlayerResponse('Z3eFGbFcaXs'),
        ...
        'streamingData': {
          'hlsManifestUrl':
              'https://manifest.googlevideo.com/api/manifest/hls_playlist/fixture/Z3eFGbFcaXs/mweb/master.m3u8',
        },
      },
      _iosPlayerResponse = {
        ...YouTubeFixtureLoader.loadPlayerResponse('Z3eFGbFcaXs'),
        ...
        'streamingData': {
          'hlsManifestUrl':
              'https://manifest.googlevideo.com/api/manifest/hls_playlist/fixture/Z3eFGbFcaXs/ios/master.m3u8',
          'adaptiveFormats': [
            {
              'mimeType': 'audio/mp4; codecs="mp4a.40.2"',
              'bitrate': 128000,
              'url': 'https://rr.googlevideo.com/audio/Z3eFGbFcaXs/english.m4a',
            },
          ],
        },
      };
...
''';
  ...
  final Map<String, dynamic> _streamlinkAndroidPlayerResponse;
  ...
  Future<String> fetchText(
    ...
    if (url.contains('fixture/Z3eFGbFcaXs/streamlink/1080p60.m3u8') ||
        url.contains('fixture/Z3eFGbFcaXs/safari/1080p60.m3u8') ||
...
''';
    ...
    if (url.contains('fixture/Z3eFGbFcaXs/streamlink/720p60.m3u8') ||
        url.contains('fixture/Z3eFGbFcaXs/safari/720p60.m3u8') ||
...
''';
    ...
    if (url.contains('fixture/Z3eFGbFcaXs/streamlink/480p30.m3u8') ||
        url.contains('fixture/Z3eFGbFcaXs/safari/480p30.m3u8') ||
...
''';
    ...
    if (url.contains('manifest.googlevideo.com') &&
        url.contains('fixture/Z3eFGbFcaXs/streamlink')) {
      return '''
#EXTM3U
  ...
  Future<Map<String, dynamic>> postPlayer({
    ...
    if (videoId == 'Z3eFGbFcaXs') {
      switch (clientProfile) {
        case YouTubePlayerClientProfile.streamlinkAndroid:
          return _streamlinkAndroidPlayerResponse;
