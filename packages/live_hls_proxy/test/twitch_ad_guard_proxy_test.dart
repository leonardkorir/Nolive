...
void main() {
      ...
      switch (request.uri.path) {
        ...
        case '/backup.m3u8':
          request.response.write(
            _cleanPlaylist(segmentPath: '/clean-segment.ts'),
          );
          break;
  ...
  test(
    'fixed quality proxy returns clean backup before slow primary',
    () async {
      upstream.listen((request) async {
        switch (request.uri.path) {
          case '/slow-primary.m3u8':
            await Future<void>.delayed(const Duration(seconds: 3));
            request.response.write(_cleanPlaylist(segmentPath: '/slow.ts'));
            break;
          case '/clean-backup.m3u8':
            request.response.write(
              _cleanPlaylist(segmentPath: '/clean-segment.ts'),
            );
            break;
      ...
      final quality = LivePlayQuality(
        ...
        metadata: {
          'twitchPlaybackGroup': TwitchPlaybackQualityGroup(
            ...
            candidates: [
              TwitchPlaybackCandidate(
                playlistUrl: '$base/slow-primary.m3u8',
                headers: const {},
                playerType: 'popout',
                platform: 'web',
                lineLabel: '默认 Popout',
              ...
              TwitchPlaybackCandidate(
                playlistUrl: '$base/clean-backup.m3u8',
                headers: const {},
                playerType: 'site',
                platform: 'web',
                lineLabel: '备用 Site',
  ...
  test(
    'auto proxy synthesizes master playlist and serves guarded variants',
    () async {
      upstream.listen((request) async {
        switch (request.uri.path) {
          ...
          case '/720-backup.m3u8':
            request.response.write(
              _cleanPlaylist(segmentPath: '/720-clean.ts'),
            );
            break;
          case '/480-primary.m3u8':
            request.response.write(
              _cleanPlaylist(segmentPath: '/480-clean.ts'),
            );
            break;
          case '/720-clean.ts':
  ...
  test(
    'auto proxy writes lower quality variants first for faster startup',
    () async {
      upstream.listen((request) async {
        switch (request.uri.path) {
          case '/1080.m3u8':
  ...
  test(
    'startup auto proxy keeps a low-latency ladder before promotion',
    () async {
      upstream.listen((request) async {
        switch (request.uri.path) {
          case '/160.m3u8':
  ...
  test(
    'auto proxy prefers non-hevc candidates for mixed codec groups',
    () async {
      upstream.listen((request) async {
        switch (request.uri.path) {
          case '/1080-hevc.m3u8':
            request.response.write(
              _cleanPlaylist(segmentPath: '/1080-hevc.ts'),
            );
            break;
          case '/1080-avc.m3u8':
      ...
      upstream.listen((request) async {
        switch (request.uri.path) {
          case '/delayed-popout.m3u8':
            popoutRequests += 1;
            request.response.write(
              _adPlaylist(segmentPath: '/delayed-popout-ad.ts'),
            );
            break;
  ...
  test(
    'fixed quality proxy rejects all-ad fallback with no live segments',
    () async {
      upstream.listen((request) async {
        switch (request.uri.path) {
          case '/only-ad.m3u8':
            request.response.write(_adPlaylist(segmentPath: '/only-ad.ts'));
            break;
          default:
            request.response.statusCode = HttpStatus.notFound;
        }
      ...
      final statusCode = await _readStatusCode(Uri.parse(wrapped.first.url));
      expect(statusCode, HttpStatus.badGateway);
    },
  );
  ...
  test(
    'fixed quality proxy keeps blank-title live segment after stripping ads',
    () async {
      upstream.listen((request) async {
        switch (request.uri.path) {
...
Future<List<int>> _consolidateHttpClientResponseBytes(
...
Future<String> _readText(Uri uri) async {
  ...
  try {
    ...
    return utf8.decode(await _consolidateHttpClientResponseBytes(response));
  } finally {
    client.close(force: true);
  }
}

Future<int> _readStatusCode(Uri uri) async {
  final client = HttpClient();
  try {
    final request = await client.getUrl(uri);
    final response = await request.close();
...
Future<List<int>> _readBytes(Uri uri) async {
  ...
  try {
    final request = await client.getUrl(uri);
    final response = await request.close();
    return await _consolidateHttpClientResponseBytes(response);
  } finally {
    client.close(force: true);
  }
}
