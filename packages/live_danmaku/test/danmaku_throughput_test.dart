...
void main() {
  test(
    'danmaku filtering and batch masking handles 100 messages per second',
    () {
      final filter = DanmakuFilterService(
        config: DanmakuFilterConfig(blockedKeywords: {'spam', 're:^BLOCK'}),
      );
      ...
      expect(
        ...
        lessThan(2000),
        reason:
            'pure Dart processing should stay well below device frame '
  ...
  test('danmaku batch mask bounds tracked duplicate keys', () {
    ...
    final firstPass = mask.allowListBatch(
      List<LiveMessage>.generate(
        5,
        (index) =>
            LiveMessage(type: LiveMessageType.chat, content: 'unique-$index'),
      ),
      now: now,
    );
    final oldestKeyAfterEviction = mask.allowListBatch(const [
