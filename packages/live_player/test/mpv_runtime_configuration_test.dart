...
class _FakeAndroidSurfacePlatform {
  _FakeAndroidSurfacePlatform(this.wid, {this.lock, Rect? rect, String? vo})
...
void main() {
  test(
    'resolveMpvRuntimeConfiguration sanitizes empty custom output values',
    () {
      ...
      expect(config.platformProperties['cache'], 'no');
      expect(config.platformProperties['cache-secs'], '0');
      expect(config.platformProperties['demuxer-max-back-bytes'], '0');
      expect(config.platformProperties['demuxer-max-bytes'], '16777216');
      expect(config.platformProperties['demuxer-readahead-secs'], '1');
  ...
  test(
    'resolveMpvRuntimeConfiguration sets ao when custom audio output is enabled',
    () {
      final config = resolveMpvRuntimeConfiguration(
        enableHardwareAcceleration: true,
        compatMode: false,
  ...
  test('resolveMpvRuntimeConfiguration keeps Android auto-safe default', () {
    final config = resolveMpvRuntimeConfiguration(
      enableHardwareAcceleration: true,
      compatMode: false,
      doubleBufferingEnabled: false,
  ...
  test(
    ...
    () {
      ...
      expect(customOutputConfig.controllerConfiguration.vo, 'mediacodec_embed');
      expect(customOutputConfig.controllerConfiguration.hwdec, 'mediacodec');
      expect(
        customOutputConfig
            .controllerConfiguration
            .androidAttachSurfaceAfterVideoParameters,
      ...
      expect(compatConfig.controllerConfiguration.vo, 'mediacodec_embed');
      expect(compatConfig.controllerConfiguration.hwdec, 'mediacodec');
      expect(
        compatConfig
            .controllerConfiguration
            .androidAttachSurfaceAfterVideoParameters,
  ...
  test(
    ...
    () {
      ...
      expect(initial.viewMountTimeout, reuse.viewMountTimeout);
      expect(initial.platformTimeout, reuse.platformTimeout);
      expect(initial.surfaceReadyBudget, const Duration(milliseconds: 800));
      expect(
        initial.surfaceReadyPollInterval,
        const Duration(milliseconds: 150),
      );
      expect(reuse.surfaceReadyBudget, const Duration(milliseconds: 350));
    },
  );

  test(
    'shouldFallbackToSafeAndroidVideoOutput only targets embedded output',
    () {
      expect(
  ...
  test(
    'usesEmbeddedAndroidMediaCodecOutput matches embedded output and default direct hardware paths',
    () {
      expect(
  ...
  test(
    'shouldWarmAndroidMediaCodecOpenPath tracks mediacodec runtime paths',
    () {
      expect(
  ...
  test(
    'waitForVideoControllerTextureReady resolves once a texture id appears',
    () async {
  ...
  test(
    'waitForVideoControllerTextureReady times out for a missing texture',
    () async {
      final textureId = ValueNotifier<int?>(null);
  ...
  test(
    'waitForVideoControllerPlatformReady resolves once controller attaches',
    () async {
  ...
  test(
    'waitForVideoControllerPlatformReady times out when attach is missing',
    () async {
      final notifier = ValueNotifier<Object?>(null);
  ...
  test(
    'tryGetAndroidSurfaceHandleListenable returns wid notifier when present',
    () {
  ...
  test(
    'tryGetAndroidSurfaceHandleListenable ignores non-android controllers',
    () {
      expect(tryGetAndroidSurfaceHandleListenable(Object()), isNull);
    },
  );
  ...
  test(
    'isAndroidSurfaceSnapshotReadyForMediaCodec requires wid publication',
    () {
      expect(
  ...
  test('resolveMpvRuntimeConfiguration applies ARC decode ladder tiers', () {
    ...
    expect(copy.androidOutputFallbackReason, 'arc-chromeos-mediacodec-copy');
    ...
    expect(soft.androidOutputFallbackReason, 'arc-chromeos-software-decode');
  ...
  test('resolveAndroidSurfacePrimeSize is resolution-agnostic on ARC', () {
    expect(resolveAndroidSurfacePrimeSize(isArcChromeOs: false), (
      width: 2,
      height: 2,
    ));
  ...
  test(
    'nextArcMpvDecodeTier walks copy → software (HW first, soft fallback)',
    () {
      expect(
  ...
  test(
    'primeAndroidVideoOutputSurface requests a tiny native surface',
    () async {
      String? methodName;
      ...
      expect(methodName, 'VideoOutputManager.SetSurfaceSize');
      expect(arguments, const <String, String>{
        'handle': '42',
        'width': '1',
        'height': '1',
      });
  ...
  test(
    'primeAndroidVideoOutputSurface accepts signed native pointer handles',
    () async {
      Map<String, String>? arguments;

      final primed = await primeAndroidVideoOutputSurface(
        playerHandle: -5476376656564322048,
        invoke: (_, args) async {
          arguments = args;
        },
      );
      ...
      expect(primed, isTrue);
      expect(arguments?['handle'], '-5476376656564322048');
    },
  );

  test(
    'primeAndroidVideoOutputSurface reports invalid or failed requests',
    () async {
      expect(
        await primeAndroidVideoOutputSurface(
          playerHandle: 0,
          invoke: (_, _) async {},
        ),
        isFalse,
      ...
      expect(
        await primeAndroidVideoOutputSurface(
          playerHandle: 42,
          invoke: (_, _) async => throw StateError('boom'),
        ),
        isFalse,
      );
    },
  ...
  test(
    'waitForFreshAndroidSurfacePublication times out on stale surface',
    () async {
  ...
  test(
    'waitForAndroidSurfaceAttachStabilization waits for controller lock',
    () async {
  ...
  test(
    'rebindAndroidVideoControllerSurface reapplies wid and vo ordering',
    () async {
      final platform = _FakeAndroidSurfacePlatform(
        ValueNotifier<int?>(8),
        lock: _FakeAsyncLock(),
  ...
  test(
    'shouldDelayAndroidEmbeddedPlayUntilSurfaceReady gates incomplete initial opens',
    () {
      expect(
  ...
  test('hasMpvStartupMediaSignal requires decoded media evidence', () {
    expect(
      hasMpvStartupMediaSignal(
        ...
        diagnostics: PlayerDiagnostics.empty(PlayerBackend.mpv),
      ),
      isFalse,
    );
    expect(
      hasMpvStartupMediaSignal(
        ...
        diagnostics: PlayerDiagnostics.empty(
          PlayerBackend.mpv,
        ).copyWith(videoParams: const {'width': '1920'}),
      ),
      isTrue,
    );
    expect(
  ...
  test(
    'resolveMpvStartupMediaSignalTimeout gives buffered sources more time',
    () {
      final defaultTimeout = resolveMpvStartupMediaSignalTimeout(
        PlaybackSource(url: Uri.parse('https://example.com/live.flv')),
      );
      final youtubeHlsTimeout = resolveMpvStartupMediaSignalTimeout(
        PlaybackSource(
          url: Uri.parse(
            'https://manifest.googlevideo.com/api/manifest/hls.m3u8',
          ),
          masterPlaylistContent: '#EXTM3U',
        ),
      );
      final externalAudioTimeout = resolveMpvStartupMediaSignalTimeout(
        PlaybackSource(
          url: Uri.parse('https://rr.googlevideo.com/videoplayback?itag=299'),
          externalAudio: PlaybackExternalMedia(
            url: Uri.parse('https://rr.googlevideo.com/videoplayback?itag=140'),
          ),
        ),
      );
      ...
      final twitchTimeout = resolveMpvStartupMediaSignalTimeout(
        PlaybackSource(
          url: Uri.parse('http://127.0.0.1:9/twitch-ad-guard/s/stream.m3u8'),
  ...
  test(
    'shouldReuseExistingAndroidSurfaceForReopen reuses a stable valid wid',
    () {
      expect(
  ...
  test(
    ...
    () {
      ...
      expect(
        classifyAndroidMediaCodecDeviceFailureReason(
          lastOpeningDoneAt: openingDoneAt,
          failureTimestamp: openingDoneAt.add(
            const Duration(milliseconds: 191),
          ),
        ),
        'mpv-vd-reinit',
      );
    ...
    () {
      ...
      expect(
        resolveAndroidEmbeddedHardwareDecoderReadyDelta(
          ...
          hardwareDecoderReadyAt: surfaceReadyAt.subtract(
      ...
      expect(
        resolveAndroidEmbeddedHardwareDecoderReadyDelta(
          surfaceReadyAt: surfaceReadyAt,
          hardwareDecoderReadyAt: surfaceReadyAt.add(
  ...
  test(
    'resolveMpvOpenPreparation derives barrier from previous state only',
    () {
      ...
      expect(freshOpen.barrierDuration, const Duration(milliseconds: 48));
      ...
      expect(sourceSwitch.barrierDuration, const Duration(seconds: 1));
    },
  );
  ...
  test('buildSplitHlsMasterPlaylistContent composes a synthetic master', () {
    ...
    expect(
      manifest,
      contains('#EXT-X-STREAM-INF:BANDWIDTH=1324000,AUDIO="audio"'),
    );
    expect(
      manifest,
      contains(
...
''';
      ...
      expect(selectedMaster, contains('GROUP-ID="audio_aac_96"'));
      expect(
        selectedMaster,
        contains(
          '#EXT-X-STREAM-INF:BANDWIDTH=2096000,RESOLUTION=960x540,AUDIO="audio_aac_96"',
      ...
      expect(selectedMaster, isNot(contains('chunklist_1_video_llhls.m3u8')));
      expect(selectedMaster, isNot(contains('chunklist_5_audio_llhls.m3u8')));
    },
  );

  test(
    'shouldInlineSplitHlsAudioIntoSource stays off for non-ll split hls',
    () {
      final source = PlaybackSource(
...
''';
  ...
  test(
    'maybeWriteResolvedSingleSourceHlsPlaylistFile localizes embedded googlevideo master',
    () async {
      const manifest = '''
#EXTM3U
#EXT-X-VERSION:6
...
''';
      final source = PlaybackSource(
        ...
        masterPlaylistContent: manifest,
      );
  ...
  test(
    'resolveMpvSourcePlatformProperties stabilizes localized googlevideo hls master',
    () {
      final source = PlaybackSource(
        url: Uri.parse(
...
#EXTM3U
#EXT-X-VERSION:6
#EXT-X-STREAM-INF:BANDWIDTH=4561186,RESOLUTION=1920x1080,CODECS="avc1.64002a,mp4a.40.2"
https://rr1---sn-qja5m5-5g.googlevideo.com/videoplayback/id/demo/itag/96/playlist/index.m3u8
''',
        hlsBitrate: '4561186',
      );
      ...
      expect(properties['cache-pause-wait'], '2');
      expect(properties['cache-pause-initial'], 'yes');
      expect(properties['demuxer-lavf-o'], contains('protocol_whitelist='));
      expect(properties['demuxer-lavf-o'], contains('live_start_index=-1'));
      expect(properties['demuxer-lavf-analyzeduration'], '3');
      expect(properties['demuxer-lavf-probesize'], '500000');
  ...
  test(
    ...
    () {
      ...
      expect(properties.containsKey('audio-files'), isFalse);
      expect(properties['cache'], 'no');
      expect(properties['cache-secs'], '0');
      expect(properties['cache-pause'], 'no');
      expect(properties['cache-pause-wait'], '1');
      expect(properties['cache-pause-initial'], 'no');
      expect(properties['demuxer-max-back-bytes'], '0');
      expect(properties['demuxer-max-bytes'], '16777216');
      expect(properties['demuxer-readahead-secs'], '1');
      expect(properties['audio-buffer'], '0.2');
  ...
  test(
    'shouldForceSeekableForSource leaves generic hls live playback linear',
    () {
      final source = PlaybackSource(
        url: Uri.parse(
  ...
  test('shouldForceSeekableForSource forces live flv seekable on Android', () {
    final source = PlaybackSource(
      url: Uri.parse(
        'https://pull-flv-q11.douyincdn.com/thirdgame/stream-407383673798656830.flv',
      ),
    ...
    expect(shouldForceSeekableForSource(source, isAndroid: true), isTrue);
  });
    ...
    () {
      ...
      expect(
        shouldIgnoreMpvErrorMessage(
          source: source,
          message:
              'ffmpeg: NULL: missing picture in access unit with size 4304',
        ),
        isTrue,
      );
    },
  ...
  test('shouldIgnoreMpvErrorMessage filters chaturbate benign mp4 warnings', () {
    final source = PlaybackSource(
      url: Uri.parse(
        'http://127.0.0.1:18080/chaturbate-llhls/demo/stream.m3u8',
      ),
  ...
  test('resolveMpvHwdecActiveSampleDelay waits until decode can engage', () {
  ...
  test(
    'resolveMpvSourcePlatformProperties caps heavy stable stream buffers',
    () {
      final source = PlaybackSource(
        url: Uri.parse('https://example.com/live.flv'),
        bufferProfile: PlaybackBufferProfile.heavyStreamStable,
  ...
  test(
    'shouldAllowUnsafePlaylistsForSource targets youtube googlevideo hls playlists',
    () {
      final manifestSource = PlaybackSource(
        url: Uri.parse(
          'https://manifest.googlevideo.com/api/manifest/hls_playlist/id/demo/playlist/index.m3u8',
        ),
      );
      final mediaPlaylistSource = PlaybackSource(
        url: Uri.parse(
          'https://rr1---sn-qja5m5-5g.googlevideo.com/videoplayback/id/demo/itag/96/playlist/index.m3u8',
        ),
      );
      final directSource = PlaybackSource(
        url: Uri.parse(
          'https://rr1---sn-qja5m5-5g.googlevideo.com/videoplayback?itag=140',
        ),
      );

      expect(shouldAllowUnsafePlaylistsForSource(manifestSource), isTrue);
      expect(shouldAllowUnsafePlaylistsForSource(mediaPlaylistSource), isTrue);
      expect(shouldAllowUnsafePlaylistsForSource(directSource), isFalse);
    },
  );

  test(
    'shouldBypassNativeMpvScreenshot skips Android embedded surface mode',
    () {
      expect(
...
''',
  ...
  test(
    'shouldRewriteSingleSourceHlsManifest enables doppiocdn low-latency hls',
    () {
      final source = PlaybackSource(
        url: Uri.parse(
