...
void main() {
  ...
  test(
    'switchable player exposes delegate current diagnostics immediately',
    () async {
  ...
  test(
    'switchable player retries delegate initialization after failure',
    () async {
      final mpvPlayer = _CapturingPlayer(
        PlayerBackend.mpv,
  ...
  test(
    'switchable player stops active delegate before backend replace',
    () async {
      final mpvPlayer = _CapturingPlayer(PlayerBackend.mpv);
  ...
  test(
    'switchable player can replace backend without replaying old source',
    () async {
      final mpvPlayer = _CapturingPlayer(PlayerBackend.mpv);
      final mdkPlayer = _CapturingPlayer(PlayerBackend.mdk);
      final player = SwitchablePlayer(
      ...
      expect(
        mdkPlayer.events,
        containsAllInOrder(['initialize', 'setVolume:1.00']),
      );
  ...
  test(
    'switchable player serializes concurrent backend replacement requests',
    () async {
  ...
  test(
    'switchable player does not build a new backend after dispose',
    () async {
      final initialStopCompleter = Completer<void>();
      final initialMpvPlayer = _CapturingPlayer(
        PlayerBackend.mpv,
