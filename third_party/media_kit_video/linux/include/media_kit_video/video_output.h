...
/**
 * Make the VideoOutput GL/EGL context current for mpv rendering / texture work.
 * Prefers GdkGLContext when present (Flutter 3.38+ H/W path).
 * No-op success when using Flutter-shared GL (no isolated context).
 */
gboolean video_output_make_context_current(VideoOutput* self);

/**
 * Complete deferred OpenGL H/W init on the raster thread (Flutter GL current).
 * Safe to call every frame; only does work while hw_pending.
 */
gboolean video_output_ensure_render_context(VideoOutput* self);

/** TRUE when mpv shares Flutter's GL context (no EGLImage context switch). */
