...
static void webview_window_plugin_handle_method_call(
  ...
  if (strcmp(method, "create") == 0) {
    ...
    // Respond first, then close on idle: destroy can re-enter Flutter method
    // channels. Look up by id in the idle callback so a raced erase cannot
    // leave a dangling WebviewWindow*.
    fl_method_call_respond_success(method_call, nullptr, nullptr);
    g_object_ref(self);
    struct CloseRequest {
      WebviewWindowPlugin *plugin;
      int64_t window_id;
    };
    auto *req = new CloseRequest{self, window_id};
    g_idle_add(
        [](gpointer user_data) -> gboolean {
          auto *request = static_cast<CloseRequest *>(user_data);
          auto *plugin = request->plugin;
          if (plugin->windows->count(request->window_id)) {
            plugin->windows->at(request->window_id)->Close();
          }
    ...
    if (fl_value_get_type(args) != FL_VALUE_TYPE_MAP) {
      fl_method_call_respond_error(
          method_call, "0", "setWebviewWindowVisibility args is not map",
          nullptr, nullptr);
      return;
    }
    auto window_id = fl_value_get_int(fl_value_lookup_string(args, "viewId"));
    ...
    if (visible_value == nullptr ||
      ...
      fl_method_call_respond_error(
          method_call, "0", "setWebviewWindowVisibility missing visible bool",
          nullptr, nullptr);
      return;
    }
    auto visible = fl_value_get_bool(visible_value);
    if (!self->windows->count(window_id)) {
      fl_method_call_respond_error(method_call, "0",
                                   "can not found webview for viewId", nullptr,
                                   nullptr);
